...
export default function ChatWidget({ 
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      const cleanText = stripMarkdown(response.text || "Sorry, I lost my connection.");
      setMessages(prev => [...prev, { role: 'model', text: cleanText }]);
    } catch (err) {
      console.error(err);
      setMessages(prev => [...prev, { role: 'model', text: "Something went wrong. Let's try again." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[1000] flex flex-col items-end">
        ...
        <div className="mb-6 w-80 md:w-96 h-[500px] md:h-[600px] glass rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-white/10 animate-in slide-in-from-bottom-8 zoom-in duration-500">
          <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] italic text-white/50">Your Career Coach</h3>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide">
            {messages.length === 0 && (
              <div className="space-y-6 py-10">
                <p className="text-3xl font-black italic text-white font-syne uppercase tracking-tighter leading-none">Need help?</p>
                <p className="text-[11px] font-bold italic text-stone-500 leading-relaxed uppercase tracking-widest">
                  {analysisContext ? "I've seen your scores. What would you like to improve?" : "Upload your resume first, then we can chat about it!"}
                </p>
              </div>
            )}
            {messages.map((m, i) => (
          ...
          <form onSubmit={handleSubmit} className="p-6 bg-black/40 border-t border-white/5 flex gap-3">
            ...
            />
            <button className="w-14 h-14 bg-emerald-600 text-white rounded-2xl flex items-center justify-center hover:bg-emerald-500 transition-all shadow-xl active:scale-95">
      ...
      <button
        ...
        className={`w-16 h-16 rounded-3xl flex items-center justify-center shadow-2xl transition-all duration-700 transform hover:scale-110 active:scale-90 ${isOpen ? 'bg-white' : 'bg-emerald-600 shadow-emerald-600/30'}`}
      >
        <i className={`fa-solid ${isOpen ? 'fa-xmark text-[#050505]' : 'fa-comment text-white'} text-xl`}></i>
      </button>
    </div>
  );
}
