...
const ChatWidget: React.FC<ChatWidgetProps> = ({ geminiService, analysisContext }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setMessages(prev => [...prev, { role: 'model', text: "Signal interrupted. Re-attempting connection." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[1000] flex flex-col items-end">
        ...
        <div className="mb-6 w-80 md:w-96 h-[500px] md:h-[600px] glass rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-white/10 animate-in slide-in-from-bottom-8 zoom-in duration-500">
          <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] italic text-white/50">Strategy Core</h3>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide">
            {messages.length === 0 && (
              <div className="space-y-6 py-10">
                <p className="text-3xl font-black italic text-white font-syne uppercase tracking-tighter leading-none">Awaiting Command.</p>
                <p className="text-[11px] font-bold italic text-stone-500 leading-relaxed uppercase tracking-widest">
                  {analysisContext ? "Analysis context loaded. Ask for deep narrative optimization or role-specific heuristics." : "Link your profile data to initiate high-fidelity career consulting."}
                </p>
              </div>
            )}
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                <div className={`max-w-[85%] px-6 py-4 rounded-[1.5rem] text-[11px] font-bold leading-relaxed italic ${m.role === 'user' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'bg-white/5 border border-white/5 text-stone-300'}`}>
                  {m.text}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleSubmit} className="p-6 bg-black/40 border-t border-white/5 flex gap-3">
            ...
            <button className="w-14 h-14 bg-emerald-600 text-white rounded-2xl flex items-center justify-center hover:bg-emerald-500 transition-all shadow-xl active:scale-95 group">
              <i className="fa-solid fa-paper-plane text-sm group-hover:rotate-12 transition-transform"></i>
            </button>
          </form>
        </div>
      )}
      ...
      <button
        ...
        className={`w-16 h-16 rounded-3xl flex items-center justify-center shadow-2xl transition-all duration-700 transform hover:scale-110 active:scale-90 ${isOpen ? 'bg-white -rotate-90' : 'bg-emerald-600 hover:bg-emerald-500 rotate-0'}`}
      >
